...
Clipboard insertion waits for shortcut modifiers to be released and prefers the app's enabled ordinary Paste menu action, regardless of its shortcut binding. Exact English/Dutch menu labels are supported initially; other labels use the existing Command-V fallback. Paste Selection, Paste and Go, Paste and Execute and similar commands are never selected by native-menu discovery. The destination and clipboard are revalidated immediately before delivery. An uncertain action is never followed by a duplicate paste. Readback still determines whether success can be claimed; genuinely unconfirmed results retain clipboard recovery.
